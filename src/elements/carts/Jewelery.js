...
import { fetchCategoriesJewleryRequest } from "../../redux/categories/categoriesActions";
...
function Jewelery() {
  ...
  const jeweleryProducts = useSelector((state) => state.categoriesJewelery);

  const navigate = useNavigate();
  //all hooks used here
  useEffect(() => {
    dispatch(fetchCategoriesJewleryRequest("jewelery"));
  }, [dispatch]);
  ...
  return (
    <Box>
      ...
      <FlexBox fw="wrap" fd="row" fjc="space-between">
        {jeweleryProducts.loading
          ? Array(4).fill(<CartLoader />)
          : jeweleryProducts.data.map((data, key) => {
              return (
                <FlexBox
                  key={key}
                  fd="column"
